...
const WomenShalwarKameez = () => {
  const { products } = useAppContext();
  const [filteredProducts, setFilteredProducts] = useState([]);

  useEffect(() => {
    const filtered = products.filter(
      (product) => product.category === "Women Shalwar Kameez"
    );
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-[#fffdf8] via-gray-50 to-[#fff7e6] px-6 pb-20 pt-10 md:px-16 md:pt-14 lg:px-32">
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#b8860b]">Signature Edit</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#1E2A38] md:text-4xl">Women Shalwar Kameez</h1>
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-gray-500">Elegant silhouettes and effortless comfort, curated for your everyday style.</p>
        </div>
...
export default WomenShalwarKameez;